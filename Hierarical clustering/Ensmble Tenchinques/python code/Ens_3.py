...
df = pd.read_excel("C:/Users/HAI/Desktop/360DigitMG/Assingment/Ensamble Technique/Datasets_ET/Coca_Rating_Ensemble.xlsx")
...
df = pd.get_dummies(df, columns = ["Company","Name","Company_Location","Bean_Type","Origin"], drop_first = True)
# drop REF, column
df.drop(['REF','Review'], axis=1, inplace=True)
df.dtypes
df.head()
df.Cocoa_Percent_new.value_counts()
...
predictors = df.loc[:, df.columns!="Rating_new"]
type(predictors)
target = df["Rating_new"]
type(target)
